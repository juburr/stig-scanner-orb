...
3. `target_base: auto` reads `/etc/os-release` from the extracted rootfs (parsed field-by-field with `awk` as untrusted data — never `source`d, because the file comes from the target image and could carry arbitrary shell), falling back to filesystem-shape hints (`apk` DB → wolfi, `rpm` DB → rhel9, dpkg → debian12). An os-release that's present but names an unsupported OS/version is authoritative and errors out; the filesystem-shape fallback only runs when there's no os-release at all (e.g. some distroless images).
...
- **RHEL host STIG against a containerized RHEL image returns 0 pass / 0 fail / ~485 N/A.** This is correct, not a defect — most STIG rules check host services that don't exist in containers. Any future change that surfaces this as a "bug" or adds workarounds in the scanner code is wrong; the right fix lives in tailoring files (queued for v1.1). Consequence for tests: the RHEL/fedora smoke jobs assert on the **N/A count and dispatch markers, never a pass count** — a `pass > N` assertion against a host-STIG profile will fail (this exact bug shipped once in `smoke-rhel9`). For pass/fail signal on the RPM path, `smoke-rhel9-cis` overrides `profile_id` to CIS Level 1, which does evaluate container-applicable rules.
...
`.circleci/test-deploy.yml` runs a matrix of integration jobs, each driving the real `stig-scanner-orb/scan` job (so the machine executor and the load → scan → summarize → store_artifacts wiring are exercised, not just the commands): `smoke-wolfi` (real signal — pass and N/A both > 50 against `cgr.dev/chainguard/static`), `smoke-gate` (verifies `fail_on_finding` + ignore-list exit codes), `smoke-ubuntu2204` and `smoke-debian12-auto` (dpkg probes; the latter also exercises `target_base: auto`), `smoke-rhel8-auto` and `smoke-rhel9` (the RPM path — compliance-operator scanner + datastream donation; `smoke-rhel8-auto` also exercises `target_base: auto` on the RPM family, and both assert the host-STIG N/A baseline, not a pass count), `smoke-rhel9-cis` (the `profile_id` override — CIS Level 1, which unlike the host STIG profile yields evaluated pass/fail signal against a container), `smoke-fedora` (second RPM datastream/profile; dispatch + complete-output assertion, no count threshold), `smoke-rootfs` (`rootfs_path` mode), and `smoke-tarball` (`image_tarball` load mode). RHEL/UBI targets pull from `registry.access.redhat.com` and fedora from `quay.io` to dodge Docker Hub anonymous pull-rate limits. All run on every push and gate publish; publish only fires on `v*` tags.

`rhel10` has **no** smoke job yet: RHEL 10 is new enough that the `stig` profile may not exist in `ssg-rhel10-ds.xml` (the orb still defaults `rhel10` → `content_profile_stig`). Before adding `smoke-rhel10`, confirm `oscap info ssg-rhel10-ds.xml` actually lists `content_profile_stig` — if it doesn't, that default in `scan.sh` is itself wrong and needs fixing first.
